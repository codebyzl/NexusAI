...
| ENG-05 | 基础设施需要手工创建 8 个容器，仓库没有 Compose 清单 | 环境难以复现，容器版本、端口、网络和启动条件容易因人而异 | 通过版本化 Compose 分阶段编排基础设施，声明持久卷、健康检查、网络和环境变量契约 | `docker compose config`；容器健康检查；数据持久化重启测试 | 设计中（FND-03） |
